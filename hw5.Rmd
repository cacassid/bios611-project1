...
heroes <- heroes %>% mutate(align = ifelse(Alignment == "bad", 0, 1))

trainIndex <- createDataPartition(heroes$align, p = .8, 
                                  list = FALSE, 
                                  times = 1)

heroes$align <- factor(heroes$align)

form <- heroes$align ~ heroes$Intelligence + heroes$Strength + heroes$Speed
