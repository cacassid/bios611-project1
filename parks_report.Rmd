...
There are hundreds of species that are listed has having some sort of concern for conservation.
...
Further analysis can also be conducted on the existing data sets. We could explore more model building techniques to find an improved model for predicting the number of mammal species. We could also explore if useful models can be generated to make other predictions, such as predicting the number of plants, the number of endangered species, or even the size of the park. We could further analyze the species data to find patterns between the number of specific species, such as the relationship between the number of preditor species and prey species. There are several possibilities for further analysis.
