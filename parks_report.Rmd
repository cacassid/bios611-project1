...
For this section of the project, I will use python to perform clustering and dimensionality reduction using the Spectral Clustering and TSNE packages from sklearn. 

In python, I performed clustering and then used TSNE to reduce the data to two dimension. The data set used contains the counts of species for 50 parks. I wanted to investigate whether or not we could isolate these parks into groups with similar characteristics based on their numbers of the different types of species.
...
These plots do not show much evidence of useful clusters. The clusters are all close to one another and often surrounded by another cluster. We don't have very much data (only 50 observations), which means that it would probably be hard to come up with good clusters.

Overall, it does not loook like we can separate our data into useful clusters that have similar features.
